...
| HiveTrace external PII seam | `hiveTracePiiBenchmark` воспроизводит pinned entity/domain corpus: 1810 processed, 0 rejected, 817 source-aligned mapped spans, 789 product-aligned spans и 850 unsupported spans. Оба split, девять domain codes и Full публикуются отдельно; [методика и команды](development.md#hivetrace-pii-benchmark) | Online/offline run 2026-09-15: Full source exact TP/FP/FN = 720/37/97, P/R/F1 = 0.95112/0.88127/0.91487; relaxed = 723/34/94. Product exact = 720/37/69. Clean FPR domain = 0/378, entity = N/A (0 cases). Это external/non-gating evidence шести mapped types; смесь не представляет production traffic, остальные три типа не покрыты. Production recognizers не менялись. |
