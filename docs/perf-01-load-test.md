...
Generated summary фиксирует измеренные числа и характеристики только своего
snapshot. Исторические результаты не являются current request/response
enforcement qualification. Статус `PERF-01` и недостающее evidence находятся в
[карте покрытия](requirements-coverage.md#нефункциональные-требования-mvp).
