...
использует thread-local или global map. Shared response workflow использует
...
только отдельным standalone `data: [DONE]`. Existing response parser работает на
blocking-safe executor и в том же parse pass строит immutable source coordinates
для ordinary JSON и SSE. Оба transport переходят в
`ResponseInspectionWorkflow`, который строит request-derived context,
оценивает independent fragments, выбирает final reaction и публикует safe RESPONSE
audit pair. `ALLOW` передаёт original source. `MASK` сверяет parser source map
и точечно патчит raw JSON string literals, включая span через несколько
