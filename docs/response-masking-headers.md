...
applies to ordinary JSON and Server-Sent Events (SSE). The normative matrix is
owned by [HTTP gateway](../spec/requirements/http-gateway.md#response-outcomes-and-headers);
exact JSON/SSE patching is owned by
[RESPONSE enforcement](../spec/requirements/response-enforcement.md#source-maps-and-exact-rewrite).
...
| Body | Original bytes | Exact source-patched bytes | Inspection JSON error | Inspection JSON error | Inspection JSON error |
...
and `503` are gateway-owned responses and contain only their inspection-error response
...
Exact error bodies and Retry-After rules belong to the
