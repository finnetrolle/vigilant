# VIG-36-04: Постоянные контракты протокола и HTTP-ошибок

**Статус:** Ready for implementation
**Epic:** [EPIC-36](../../epics/epic_36_current_requirements_and_work_item_lifecycle.md)
**Ветка:** migrate по wire protocol contract
**Зависит от:** [VIG-36-01](issue_36_01_completion_workflow.md)
**Блокирует:** [VIG-36-05](issue_36_05_request_enforcement_contracts.md), [VIG-36-06](issue_36_06_response_gateway_contracts.md), [VIG-36-08](issue_36_08_current_catalog_closure.md)
**Связанные требования:** `MVP-07`, `PROXY-01`, `PROXY-02`, `PROXY-03`
**Оценка:** 1-2 инженерных дня
**Уверенность:** Medium

## Результат

Chat Completions request/ordinary JSON/SSE parsing и closed HTTP error matrix доступны через permanent protocol/error owners. REQUEST migration получает стабильные field maps и status/body references.
...
VIG-36-06 дополняет http-gateway.md transport/lifecycle sections, не переписывая shared error table. Domain/policy reactions остаются у VIG-36-05/06.

## Source batch: 5 issues; ни один baseline epic не удаляется
...
- `spec/issues/epic_06/issue_06_01_protocol_contract.md`
- `spec/issues/epic_06/issue_06_02_chat_completions_request_parser.md`
- `spec/issues/epic_06/issue_06_03_chat_completions_response_parser.md`
- `spec/issues/issue_14_strict_protocol_gap_outcomes.md`
- `spec/issues/issue_29_openai_error_contract.md`

## Destination и current consumers

Permanent destinations:

- `spec/requirements/chat-completions-protocol.md`
- `spec/requirements/http-gateway.md`: только error/descriptor sections
...
Source consumers для фактической сверки: ChatCompletionsRequestParser, ChatCompletionsResponseParser, request/response descriptor validation и existing protocol conformance / OpenAI error E2E tests.

## Обязательные contract cases

1. Supported request descriptor и explicit versioned schema, normalized operation/phase/transport, unknown-field preservation и no reserialization.

2. Полная recognized request field map, JSON Schema walker, provenance/source coordinates, independent fragments, duplicate keys, malformed UTF-8/JSON, ambiguous/uninspectable shapes, depth/fragment bounds и recognized inspection gaps.

3. Ordinary JSON response: каждый разрешённый terminal/content-bearing shape, unknown fields, tool/refusal/output fragments и failure semantics.

4. SSE: framing, UTF-8/event boundaries, cross-event source maps, standalone data: [DONE], EOF versus terminal semantics, incomplete/malformed/ambiguous/interrupted cases. Parsing не начинает enforcement и не раскрывает response bytes.

5. Все пять outcomes VIG-29: exact status, body shape, message/type/code, Retry-After presence/value и privacy; request и response consumers используют одну таблицу, когда semantics совпадают.

6. Публичный MVP остаётся Chat Completions. Responses/Realtime/Batch не получают новых schema, terminal semantics или implementation issues.
...
EPIC-06 не входит в removal batch. Перенести его current Chat Completions clauses в новый protocol owner и убрать completed child references. Сохранить только оставшийся future OpenAI Responses scope вне MVP; поскольку для него ещё нет implementation-ready leaves, оформить оставшийся epic как Draft с этим объяснением и синхронно обновить registry. Это изменение planning state, а не утверждение завершения Responses. Realtime/Batch остаются неактивированными post-MVP placeholders, без новых contracts. Использовать этот current-only remainder, а не сохранять 3/3 historical checklist.
...
Реализация/изменение parser, новый protocol provider/surface, изменение HTTP errors, response enforcement или автоматическое закрытие EPIC-06.
