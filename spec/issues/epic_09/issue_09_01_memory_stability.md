...
- [ ] Исходный отказ воспроизведён полным PERF-01 профилем либо сохранённый
...
- [ ] Heap/native-memory/JFR или эквивалентный профиль локализует retained или
  unbounded resource class до изменения production code.
- [ ] Исправление ограничено найденной причиной и сохраняет streaming,
  cancellation, timeout и header semantics.
- [ ] При baseline `-Xms512m -Xmx512m` proxy warm-up и measurement завершаются
  без OOM, accept/connect collapse и request failures.
- [ ] Memory после warm-up остаётся bounded по зафиксированной методике; один
  только увеличенный heap без объяснения root cause не считается решением.
- [ ] `docs/perf-01-result.md` дополнен фактической OOM-причиной исходного run и
  ссылкой на новый evidence/result.
- [ ] Focused regression tests и `./gradlew build` проходят.
