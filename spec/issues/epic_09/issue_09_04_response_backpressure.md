...
- [x] Client запрашивает response data по одному или малому фиксированному
  числу элементов вместо `Long.MAX_VALUE`.
- [x] Controllable upstream publisher наблюдает bounded outstanding demand и
  не выпускает весь response до разрешения client.
- [x] После последовательного release всех permits client получает полный
  content в правильном порядке и exchange завершается успешно.
- [x] Test чувствителен к unbounded prefetch/buffering и сохраняет существующие
  TTFB/SSE assertions.
