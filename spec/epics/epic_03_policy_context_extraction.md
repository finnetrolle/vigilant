...
VIG-27, offline JWT из VIG-28 и trusted Bridge External из VIG-30. JWT проверяет
pinned RS256 trust snapshot, а External выполняет bounded async lookup; все три
mode передают assembler-у только normalized identity.
...
в development/test и требует configured user/optional groups. JWT требует
exact issuer, audience и non-empty pinned RSA public JWK set с unique `kid`.
`EXTERNAL` требует trusted absolute HTTP(S) Bridge endpoint и выполняет один
async lookup. JWT и `EXTERNAL` разрешены в production. User/groups используют ASCII grammar
...
по signature/issuer/audience/time contract до чтения identity claims без
network I/O. External требует non-empty token и передаёт его только trusted
Bridge в пределах whole-exchange deadline. Missing/non-Bearer получает typed authentication
...
- Пользователь и группы поступают в normalized contract из выбранного
  ...
  top-level `groups`, External - strict Bridge response.
